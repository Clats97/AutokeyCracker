...
def gui_crack():
    ...
    passkey_path = passkey_entry.get().strip()
    passkey_path = passkey_path.replace('"', '').replace("'", "")
    if not passkey_path:
        passkey_path = "passkeys.txt"
    if not ciphertext:
    ...
    if not os.path.isfile(passkey_path):
        output_text.insert(tk.END, f"Passkeys file not found at: {passkey_path}\n")
        return
    with open(passkey_path, "r") as f:
    ...
    for key in keys:
        ...
        if is_english_sentence(plaintext):
            ...
            output_text.insert(tk.END, "Candidate Keyword: " + repr(key) + "\n")
...
root = tk.Tk()
root.title("Vigenere Autokey Cracker GUI v1.00")
...
ascii_label = tk.Label(header_frame, text=ascii_art_text, fg="red", font=mono_font, justify="left")
ascii_label.pack(anchor="w")
title_frame = tk.Frame(header_frame)
title_frame.pack(anchor="w", pady=(2, 0))
title_label = tk.Label(title_frame, text="V I G E N E R E   A U T O K E Y   C R A C K E R", fg="blue", font=mono_font)
...
passkey_frame.grid(row=2, column=1, sticky="w", padx=2, pady=2)
passkey_entry = tk.Entry(passkey_frame, width=40)
passkey_entry.insert(0, "Enter Path Here")
passkey_entry.grid(row=0, column=0)
passkey_paste_button = tk.Button(passkey_frame, text="Paste", command=paste_clipboard_passkey)
passkey_paste_button.grid(row=0, column=1, padx=5)
